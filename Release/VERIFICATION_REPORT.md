# PinChat 2.2 桌面任务联动版验证报告

验证日期：2026-09-15（Asia/Shanghai）

## 交付物

- `PinChat.app`：macOS 14+、Apple Silicon、本机 ad-hoc 签名的可运行应用，版本 0.2.2（4）。
- `PinChat-source.zip`：完整 Swift Package / Xcode 工程源码，不含 Git、构建缓存和应用产物。
- v1.0、v1.1、v2.0、v2.1、v2.2 五份只读冻结需求及各自 SHA-256 校验文件。
...
- 对话完成卡的 `↗` 会取消 PinChat 对当前线程的订阅，然后使用 `codex://threads/<thread-id>` 打开同一任务。
- v2.2 不再终止只读 App Server；交接后仍可继续轮询桌面任务进度，同时不会重新占用已交接线程。
